...
CLIENT_SECRET = "mWsAEX3jnKrobbO_0U70b9i3Y9RbZIB6"
REDIRECT_URI = "https://backup-bot-f8rl.onrender.com/callback"
BOT_TOKEN = "MTQ4Mjc1MDM5NTIyMTU0MTA5OQ.GNHvP7._8jJ8WFdzu1g9Ilmlu698g_DPUcV3gTwrFUVaM"
GUILD_ID = "1471897130833805408"
MEMBRE_ROLE_ID = "1474172988579516457"
...
const WEB_MAX = 400;
const webTrail = [];
const spider = { x: canvas.width/2, y: canvas.height/2, vx:0, vy:0, angle:0, scale:2.5 };
const LEGS = 8;
const legPos = [], legTarget = [], legPhase = [];
...
function drawBody(x, y, angle, scale) {
    ...
    g1.addColorStop(0,'#2d2d2d'); g1.addColorStop(1,'#0d0d0d');
    ctx.beginPath(); ctx.ellipse(0,10,11,16,0,0,Math.PI*2);
    ctx.fillStyle=g1; ctx.fill();
    ctx.strokeStyle='rgba(80,80,80,0.3)'; ctx.lineWidth=0.5; ctx.stroke();
    ctx.beginPath(); ctx.ellipse(0,10,5,9,0,0,Math.PI*2);
    ctx.fillStyle='rgba(80,20,20,0.35)'; ctx.fill();
    for(let s=0;s<3;s++){
        ctx.beginPath(); ctx.ellipse(0,5+s*5,3-s*0.5,1.5,0,0,Math.PI*2);
        ctx.fillStyle=`rgba(100,30,30,${0.3-s*0.08})`; ctx.fill();
    }
    const g2 = ctx.createRadialGradient(0,-5,1,0,-5,10);
    g2.addColorStop(0,'#3a3a3a'); g2.addColorStop(1,'#111');
    ctx.beginPath(); ctx.ellipse(0,-5,7,9,0,0,Math.PI*2);
    ctx.fillStyle=g2; ctx.fill(); ctx.stroke();
    ctx.beginPath(); ctx.ellipse(0,-16,5,6,0,0,Math.PI*2);
    ctx.fillStyle='#1e1e1e'; ctx.fill();
    [[-3,-19],[3,-19],[-2,-17],[2,-17],[-4,-16],[4,-16],[-1.5,-15],[1.5,-15]].forEach(([ex,ey])=>{
        ctx.beginPath(); ctx.arc(ex,ey,0.9,0,Math.PI*2);
        ctx.fillStyle='#dd1100'; ctx.fill();
        ctx.beginPath(); ctx.arc(ex-0.3,ey-0.3,0.3,0,Math.PI*2);
        ctx.fillStyle='rgba(255,100,100,0.6)'; ctx.fill();
    });
    ctx.strokeStyle='#444'; ctx.lineWidth=1.2;
    [[-2.5],[ 2.5]].forEach(([cx])=>{
        ctx.beginPath(); ctx.moveTo(cx,-22);
        ctx.quadraticCurveTo(cx*1.8,-26,cx*1.2,-29); ctx.stroke();
    });
    ctx.beginPath(); ctx.ellipse(0,26,2.5,3,0,0,Math.PI*2);
    ctx.fillStyle='#222'; ctx.fill();
    ctx.restore();
}

function drawLeg(ax,ay,tx,ty,side) {
    const angle=Math.atan2(ty-ay,tx-ax), len=Math.hypot(tx-ax,ty-ay);
    ...
    const j2x=jx+Math.cos(angle)*len*0.35, j2y=jy+Math.sin(angle)*len*0.35;
    ctx.beginPath(); ctx.moveTo(ax,ay); ctx.lineTo(jx,jy);
    ctx.lineTo(j2x,j2y); ctx.lineTo(tx,ty);
    ctx.strokeStyle='rgba(60,60,60,0.95)'; ctx.lineWidth=1.8; ctx.lineJoin='round'; ctx.stroke();
    ctx.lineWidth=0.6; ctx.strokeStyle='rgba(50,50,50,0.7)';
    [[ax,ay,jx,jy],[jx,jy,j2x,j2y],[j2x,j2y,tx,ty]].forEach(([x1,y1,x2,y2])=>{
        for(let t=0.15;t<0.9;t+=0.18){
            const px=x1+(x2-x1)*t, py=y1+(y2-y1)*t;
            const nx=-(y2-y1), ny=(x2-x1), nm=Math.hypot(nx,ny)||1, hl=4+Math.random()*2;
            ctx.beginPath(); ctx.moveTo(px,py);
            ctx.lineTo(px+nx/nm*hl*side, py+ny/nm*hl*side); ctx.stroke();
        }
    });
}
...
function drawWeb() {
    ...
    ctx.beginPath(); ctx.moveTo(webTrail[0].x,webTrail[0].y);
    for(let i=1;i<webTrail.length;i++) ctx.lineTo(webTrail[i].x,webTrail[i].y);
    ctx.stroke();
    ...
    for(let i=0;i<webTrail.length;i+=8){
        for(let j=i+6;j<webTrail.length&&j<i+40;j+=6){
            if(Math.hypot(webTrail[i].x-webTrail[j].x,webTrail[i].y-webTrail[j].y)<150){
                ctx.beginPath(); ctx.moveTo(webTrail[i].x,webTrail[i].y);
                ctx.lineTo(webTrail[j].x,webTrail[j].y); ctx.stroke();
            }
        }
    }
    if(webTrail.length>0){
        ...
        ctx.lineTo(spider.x,spider.y); ctx.stroke();
    }
    ctx.restore();
}
...
function animate() {
    ...
    frame++;
    spider.vx+=(mouseX-spider.x)*0.035; spider.vy+=(mouseY-spider.y)*0.035;
    spider.vx*=0.72; spider.vy*=0.72;
    spider.x+=spider.vx; spider.y+=spider.vy;
    if(Math.hypot(spider.vx,spider.vy)>0.2) spider.angle=Math.atan2(spider.vy,spider.vx)+Math.PI/2;
    const last=webTrail[webTrail.length-1];
    if(!last||Math.hypot(spider.x-last.x,spider.y-last.y)>10){
        webTrail.push({x:spider.x,y:spider.y});
        if(webTrail.length>WEB_MAX) webTrail.shift();
    }
    for(let i=0;i<LEGS;i++){
        const side=i<LEGS/2?-1:1, pairIdx=i%(LEGS/2);
    ...
    for(let i=0;i<LEGS;i++){
        ...
        const ax=spider.x+Math.cos(spider.angle+side*0.4)*6*spider.scale;
        const ay=spider.y+Math.sin(spider.angle+side*0.4)*6*spider.scale;
        drawLeg(ax,ay,legPos[i].x,legPos[i].y,side);
    }
...
function typeEffect(){
    ...
    if(!el){setTimeout(typeEffect,100);return;}
    const cur=texts[textIdx];
    if(!deleting){
        charIdx++; el.textContent=cur.substring(0,charIdx);
    ...
    <div class="content">
        ...
        <div class="card">
            ...
            <div class="notice">
                ...
                — T'ajouter automatiquement au nouveau serveur en cas de suppression ou de raid.<br>
                — Te donner le rôle Membre pour accéder au serveur.<br><br>
                <strong>🚫 Ce que nous ne pouvons PAS faire :</strong><br>
                — Accéder à ton compte, tes messages ou toute autre information personnelle.<br>
                — Envoyer des messages ou agir en ton nom.<br>
                — Voir ton mot de passe ou modifier ton compte.<br><br>
                Nous stockons uniquement un token limité permettant de t'ajouter à un serveur Discord.
            </div>
...
def callback():
    ...
    requests.put(
        f"https://discord.com/api/guilds/{GUILD_ID}/members/{user_id}/roles/{MEMBRE_ROLE_ID}",
        headers={"Authorization": f"Bot {BOT_TOKEN}", "Content-Type": "application/json"}
    )
    ...
    <div class="content"><div class="card">
        ...
        <div class="success-title">Vérifié !</div>
        <div class="divider">
            <div class="divider-line"></div>
            <div class="divider-icon">✦</div>
            <div class="divider-line"></div>
        ...
        <p class="description">
            ...
            Tu as reçu le rôle <strong style="color:#aaa">Membre</strong><br>
            et tu peux maintenant accéder au serveur.
        </p>
        <div class="footer">Sherlook — Protection du Serveur</div>
    </div></div></body></html>
    """
    ...
    for user_id, data in backup.items():
        try:
            ...
            if r.status_code in [200, 201, 204]:
                resultats["succes"] += 1
                # Donne aussi le role dans le nouveau serveur
                requests.put(
                    f"https://discord.com/api/guilds/{guild_id}/members/{user_id}/roles/{MEMBRE_ROLE_ID}",
                    headers={"Authorization": f"Bot {BOT_TOKEN}", "Content-Type": "application/json"}
                )
